...
def land_top_grid_analysis(grid_size, full_site, land_extents_for_analysis, blender_file_path):
    ...
    if land is not None:
        ...
        vtx = [[i * land.dimensions.x / land_x_dimension + land.location.x - land.dimensions.x / 2.0,
                ...
                land.dimensions.z + height_of_grid_above_site_plus_site_z_bound] for i in range(land_x_dimension + 1)
               for j in range(land_y_dimension + 1)]

        faces = [[i * (land_y_dimension + 1) + j, (i + 1) * (land_y_dimension + 1) + j,
                  (i + 1) * (land_y_dimension + 1) + j + 1] for i in
                 range(land_x_dimension) for j in range(land_y_dimension)]
        faces.extend(
            [[i * (land_y_dimension + 1) + j, (i + 1) * (land_y_dimension + 1) + j + 1,
              i * (land_y_dimension + 1) + j + 1] for i in range(land_x_dimension)
...
def site_analysis3_calls_for_land_top_grid_function(grid_size, full_site, land_extents_for_analysis, building_for_analysis, level_for_implantation,
    ...
    top_grid_vtx, land_x_dimension, land_y_dimension, height = land_top_grid_analysis(grid_size, full_site,
                                                                                      land_extents_for_analysis,
                                                                                      blender_file_path,
                                                                                      slp_app_file_path)
    ...
    for i in range(land_x_dimension):
        for j in range(land_y_dimension):
            faces.append([i * (land_y_dimension + 1) + j, (i + 1) * (land_y_dimension + 1) + j,
                          (i + 1) * (land_y_dimension + 1) + j + 1])
            faces.append([i * (land_y_dimension + 1) + j, (i + 1) * (land_y_dimension + 1) + j + 1,
                          i * (land_y_dimension + 1) + j + 1])
